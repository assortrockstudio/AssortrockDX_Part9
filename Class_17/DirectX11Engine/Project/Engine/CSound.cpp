...
CSound::~CSound()
{
	if (nullptr != m_pSound)
	{
		FMOD_RESULT result = m_pSound->release();
		m_pSound = nullptr;
	}
}
...
int CSound::Play(int _iRoopCount, float _fVolume, bool _bOverlap)
{
	...
	FMOD_System->playSound(m_pSound, nullptr, false, &pChannel);
...
int CSound::Load(const wstring& _strFilePath)
{
	...
	if (FMOD_OK != FMOD_System->createSound(path.c_str(), FMOD_DEFAULT, nullptr, &m_pSound))
	{
		assert(nullptr);
	}
